    ...
    <div className="flex flex-col gap-7">
      ...
      <div className="flex items-center gap-3">
        ...
        {onSale && variant?.compareAtPrice && (
          <>
            <s className="font-mono text-base text-mist tabular">
              {formatMoney(variant.compareAtPrice.amount, currencyCode)}
            </s>
            {off != null && (
              <span className="eyebrow rounded-sm bg-accent-strong px-2 py-1 leading-none text-white">
                −{off}%
              </span>
            )}
          </>
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      !possible && !active ? 'text-mist line-through decoration-1' : '',
                    ].join(' ')}
                  >
      ...
      <div className="flex flex-col gap-3">
        <button
          ...
          className="flex h-14 w-full items-center justify-center gap-2 rounded-md bg-ink text-base font-medium text-snow transition-fluid hover:bg-graphite disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <button
          ...
          onClick={handleBuy}
          disabled={!available || adding || buying}
          className="flex h-14 w-full items-center justify-center gap-2 rounded-md bg-accent-strong text-base font-medium text-white transition-fluid hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
        >
      ...
      <p className="flex items-center gap-2 text-xs text-graphite">
        <span
          className={`inline-block h-2 w-2 rounded-full ${available ? 'bg-success' : 'bg-mist'}`}
        />
